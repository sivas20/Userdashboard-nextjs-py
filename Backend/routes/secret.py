...
# GET ALL SECRETS
@secret_routes.route("/allsecrets", methods=["GET"])
def get_secrets():
    user_id = session.get("user_id")
    ...
    for s in secrets:
        # decrypt
        decrypted_message = cipher.decrypt(s.content.encode()).decode()
...
def get_secret(secret_id):
    ...
    decrypted_message = cipher.decrypt(secret.content.encode()).decode()
